            ...
            <table class="table align-middle table-row-dashed fs-6 gy-5" id="kt_ecommerce_sales_table">
              <thead>
                <tr class="text-start text-gray-500 fw-bold fs-7 text-uppercase gs-0">
                  ...
                  <th class="text-center min-w-200px">Used By</th>
              ...
              <tbody class="fw-semibold text-gray-600">
                @foreach ($coupons as $coupon)
                  <tr>
                    ...
                    @php
                      $totalUsed = $coupon->orders
                          ->filter(function ($order) {
                              return $order->status !== 'unpaid';
                          })
                          ->count();
                    @endphp
                    <td class="pe-0 text-center" data-order="{{ $totalUsed }}">
                      <!--begin::Badges-->
                    ...
                    @endphp
                    <td class="text-center" data-order="{{ $totalDiscount }}">
                      <span class="fw-bold text-success">₹{{ $totalDiscount }}
                      </span>
                    </td>
                    @php
                      $couponOrders = $coupon->orders->filter(function ($order) {
                          return $order->status !== 'unpaid';
                      });
                    @endphp
                        ...
                        @foreach ($couponOrders as $order)
                          @if ($loop->index < 2)
                            <div class="symbol symbol-circle symbol-35px" data-bs-toggle="tooltip"
                              ...
                              @if ($order->user->customer->avatar)
                                <img src="{{ asset($order->user->customer->avatar) }}" alt="" />
                              @else
                          ...
                          @endif
                        @endforeach
                        @if (count($couponOrders) > 2)
                          <a href="#" class="symbol symbol-35px symbol-circle" data-bs-toggle="tooltip"
                            data-bs-custom-class="tooltip-inverse" data-bs-placement="top" data-bs-html="true"
                            title="View Items <span class='badge badge-primary'>{{ count($couponOrders) - 2 }}</span>">
                            <span
                              class="symbol-label bg-dark text-gray-900 fs-8 fw-bold">+{{ count($couponOrders) - 2 }}</span>
                          </a>
                        @endif
                      </div>
                    </td>
                    <td class="text-end">
                      <div class="form-check form-switch form-check-custom form-check-success form-check-solid">
                        ...
                        <a href="/dashboard/coupons/{{ $coupon->id }}/edit" class="mx-3 ms-5">Edit</a>
