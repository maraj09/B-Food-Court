...
class WelcomeEmail extends Mailable implements ShouldQueue
{
    ...
    {
        $this->points = Point::first();
        $this->settings = Setting::first();
        $this->user = User::where('id', $userId)->first();

        $template = EmailTemplate::where('trigger', 'welcome')->first();
        ...
        $this->content = str_replace(
            ...
            [$this->user->name,  $this->user->email,  $this->user->phone, $this->user->id, $this->user->point->points, $this->user->customer->date_of_birth, 0, '', 0, ''],
            $template->body
        );
